...
from wenet.utils.checkpoint import load_checkpoint, load_trained_modules, migration
from wenet.finetune.lora.encoder import (LoRATransformerEncoder, LoRAConformerEncoder)
...
def init_model(args, configs):
    ...
    if 'encoder_init' in configs:
        encoder = migration(encoder, configs['encoder_init'])
    
    decoder = WENET_DECODER_CLASSES[decoder_type](vocab_size,
                                                  encoder.output_size(),
                                                  **configs['decoder_conf'])

    if 'decoder_init' in configs:
        decoder = migration(decoder, configs['decoder_init'])
