...
def test():
    ...
    data_type = 'raw'
    data_list_file = 'data/train/data.{}.list'.format(data_type)
    # data_list_file = 'data/230423_test/data.list'
    config_file='conf/finetune_whisper_largev3.yaml'
    ...
    dataset = Dataset(data_type, data_list_file, tokenizer, configs['dataset_conf'], partition=True)
    # data_loader =  DataLoader(dataset, num_workers=1, persistent_workers=True, generator=generator)
    data_loader =  DataLoader(dataset, num_workers=0, generator=generator)
    for batch_idx, batch_dict in enumerate(data_loader):
        print('batch_idx:', batch_idx)
        print('feat_shape', batch_dict['feats'].shape)
