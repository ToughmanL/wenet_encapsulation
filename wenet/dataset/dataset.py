...
def Dataset(data_type,
    ...
    elif feats_type == 'w2vbert_fbank_conf':
        fbank_conf = conf.get('w2vbert_fbank_conf', {})
        dataset = dataset.map(partial(processor.compute_w2vbert_fbank, **fbank_conf))
...
def test():
    ...
    data_type = 'shard'
    data_list_file = 'data/dev/data.shard.list'
    config_file='conf/wenet_w2vbert.yaml'

    with open(config_file, 'r') as fin:
        configs = yaml.load(fin, Loader=yaml.FullLoader)
    tokenizer = init_tokenizer(configs)

    datasetconfigs = configs['dataset_conf']
    generator = torch.Generator()
    dataset = Dataset(data_type, data_list_file, tokenizer, datasetconfigs, partition=True)
