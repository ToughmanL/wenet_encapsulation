...
def main():
    ...
    for mode in args.modes:
        ...
        # 添加confidence文件
        confidence_name = os.path.join(dir_name, 'confidence')
        files[mode + 'confidence'] = open(confidence_name, 'w')
    ...
    with torch.cuda.amp.autocast(enabled=True,
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(test_data_loader):
                ...
                # for i, key in enumerate(keys):
                #     for mode, hyps in results.items():
                #         for tokens in hyps[i].nbest:
                #             confidence = str(hyps[i].confidence)
                #             line = '{} {}'.format(key, tokenizer.detokenize(tokens)[0])
                #             confi_line= '{} {}'.format(key, confidence)
                #             logging.info('{} {}'.format(mode.ljust(max_format_len),
                #                                         line))
                #             files[mode].write(line + '\n')
                #             files[mode + 'confidence'].write(confi_line + '\n')
